...
export function registerFeishuTaskTaskTool(api: OpenClawPluginApi): void {
  ...
  registerTool(
    api,
    {
      ...
      description:
        "【以用户或应用身份】飞书任务管理工具。用于创建、查询、更新任务。Actions: create（创建任务）, get（获取任务详情）, list（查询任务列表，仅返回我负责的任务）, patch（更新任务）, add_members（添加任务成员）。时间参数使用ISO 8601 / RFC 3339 格式（包含时区），例如 '2024-01-01T00:00:00+08:00'。支持通过 auth_type 参数切换用户(user)或应用(tenant)身份。create 与 patch 支持通过 custom_fields 设置自定义字段（难度、优先级、数字、成员、日期、单选、多选、文本），字段 guid 与选项 guid 需预先从飞书任务清单配置获取。",
